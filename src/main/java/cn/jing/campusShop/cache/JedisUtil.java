...
/**
 * function:Redis工具类，封装好Redis的连接以及与键值对相关的操作 注意：操作完Jedis实例对象后要记得close关闭，以释放相应的资源
 * 
 * @author liangjing
 *
 */
public class JedisUtil {
	...
	/** Redis连接池对象 (在spring-redis.xml配置文件中进行注入) */
	...
	/**
	 * 从jedis连接池中获取获jedis对象（Redis的客户端--针对于Java语言）
	 * 
	 * @return
	 */
	...
	public class Keys {
		...
		public String rename(String oldkey, String newkey) {
			// SafeEncoder.encode将字符串转换为UTF-8编码的字节数组
			return rename(SafeEncoder.encode(oldkey), SafeEncoder.encode(newkey));
		}

		/**
		 * 更改key名称,仅当新key不存在时才执行
		 * 
		 * @param String
		 *            oldkey
		 * @param String
