...
$(function() {
	...
	// 更新商品信息的URL
	var productPostUrl = '/campusShop/shopadmin/modifyproduct';
	// 由于商品添加和编辑使用的是同一个页面，
	// 该标识符用来标明本次是添加还是编辑操作
	var isEdit = false;
	if (productId) {
		...
		getCategory();
		productPostUrl = '/campusShop/shopadmin/addproduct';
	}
	...
	$('#submit').click(
		function() {
			...
			$.ajax({
				url : productPostUrl,
				type : 'POST',
				//注意：此处与productcategorymanagement.js的处理方式不一样。这里的键data的值为formData，而formData中并不是都是转化为json字符串来传递的，所以contentType的值为false
				data : formData,
				contentType : false,
				processData : false,
				cache : false,
