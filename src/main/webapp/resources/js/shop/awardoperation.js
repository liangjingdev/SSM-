$(function() {
	...
	//通过awardId获取奖品信息的URL
	var infoUrl = '/campusShop/shopadmin/getawardbyid?awardId=' + awardId;
	//更新奖品信息的URL
	var awardPostUrl = '/campusShop/shopadmin/modifyaward';
	//由于奖品添加和编辑使用的是同一个页面
	//所以该标志符用来标明本次是添加还是编辑操作
	var isEdit = false;
	if (awardId) {
		//若有awardId则为编辑操作
		getInfo(awardId);
		...
		awardPostUrl = '/campusShop/shopadmin/addaward';
	}
