...
$(function() {
	...
	var pageSize = 3;
	// 获取店铺列表的URL
	var listUrl = '/campusShop/frontend/listshops';
	// 获取店铺类别列表以及区域列表的URL
	var searchDivUrl = '/campusShop/frontend/listshopspageinfo';
	// 页码
	var pageNum = 1;
	...
	function addItems(pageSize, pageIndex) {
		...
		$.getJSON(url, function(data) {
			if (data.success) {
				...
				maxItems = data.count;
				var html = '';
				// 遍历店铺列表，拼接出卡片集合(new Date(item.lastEditTime).Format("yyyy-MM-dd")调用的是common.js中的Format方法)
	...
	$('.shop-list').on('click', '.card', function(e) {
		...
		window.location.href = '/campusShop/frontend/shopdetail?shopId=' + shopId;
	});
